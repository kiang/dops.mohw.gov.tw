...
class TaiwanOpenDataCrawlerOlder:
    def __init__(self):
    ...
    def extract_ods_links_from_api(self, api_data):
        ...
        for dist in distributions:
            ...
            if '.ods' in download_url.lower():
                ...
                # Look for date pattern like 1030101-1031231
                date_match = re.search(r'(\d{3})0101-\d{7}', decoded_filename)
                if date_match:
                    year = date_match.group(1)
                else:
                    # Try other patterns
                    year_patterns = [
                        r'^(\d{3})',  # Start with 3 digits
                        r'(\d{3})(?=\D)',  # 3 digits followed by non-digit
                    ]
                    
                    year = None
                    for pattern in year_patterns:
                        year_match = re.search(pattern, decoded_filename)
                        if year_match:
                ...
                if not year:
                    ...
                    year_match = re.search(r'(\d{3})年', description)
                    if year_match:
                        year = year_match.group(1)
                
                # Convert ROC year to AD year
                if year and len(year) == 3 and year.isdigit():
    ...
    def process_dataset(self, dataset_id, target_dir=None):
        ...
        # Print dataset info
        dataset_name = api_data['result'].get('name', 'Unknown')
    ...
    def run(self):
        """Run the crawler for older datasets"""
        # Dataset mapping to their respective newer dataset directories
        dataset_mapping = {
            '147093': 'raw/157217',  # Older family violence data -> merge with 157217
            '147094': 'raw/152223',  # Older sexual assault data -> merge with 152223
            '147095': 'raw/156470'   # Older child protection data -> merge with 156470
        }
        
        for dataset_id, target_dir in dataset_mapping.items():
            self.process_dataset(dataset_id, target_dir)
        
        print("\nCrawling completed!")

if __name__ == "__main__":
    # Create crawler instance and run
    crawler = TaiwanOpenDataCrawlerOlder()
    crawler.run()